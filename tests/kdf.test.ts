...
describe("Mountain KDF (Key Derivation Function)", () => {
  ...
  it("derives an identical AES-GCM-256 key given the same passphrase and salt (Deterministic)", async () => {
    ...
    // Use 10,000 iterations in unit tests for fast execution
    const bundle1 = await deriveVaultKey(passphrase, salt, 10_000);
    const bundle2 = await deriveVaultKey(passphrase, salt, 10_000);
  ...
  it("derives completely different keys when salts differ (Rainbow Table Protection)", async () => {
    ...
    const bundleA = await deriveVaultKey(passphrase, saltA, 10_000);
    const bundleB = await deriveVaultKey(passphrase, saltB, 10_000);
