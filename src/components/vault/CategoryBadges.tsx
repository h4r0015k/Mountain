...
export const RecordItemIconBadge: React.FC<{
  ...
  if (type === 'LOGIN' && url) {
    return <FaviconBadge url={url} fallbackText={title} size={size} allowExternalFetch={allowExternalFetch} />;
  }

  const sizeClass =
    size === 'sm' ? 'w-7 h-7 text-xs rounded-md' : size === 'lg' ? 'w-11 h-11 rounded-xl' : 'w-9 h-9 rounded-lg';
  ...
  switch (type) {
    case 'CARD':
      return (
        <div className={`${sizeClass} bg-amber-950/40 border border-amber-600/30 flex items-center justify-center shrink-0`}>
          <CreditCard className={`${iconClass} text-amber-400`} />
        </div>
      );
    case 'API_KEY':
      return (
        <div className={`${sizeClass} bg-indigo-950/40 border border-indigo-600/30 flex items-center justify-center shrink-0`}>
          <Terminal className={`${iconClass} text-indigo-400`} />
        </div>
      );
    case 'SECURE_NOTE':
      return (
        <div className={`${sizeClass} bg-zinc-900 border border-zinc-800 flex items-center justify-center shrink-0`}>
